...
5. **Target Projects**:
   ...
   - Distribution artifacts copied by agentsmgr populate command
   - Downstream managed entries are file-level `.git/info/exclude` entries
...
.. code-block::
    ...
    agents-common (components/) → agentsmgr generate →
    agents-common (distribution/) → Tag Release (agents-N) →
    agentsmgr populate →
...
.. code-block::
    ...
    agentsmgr generate →
    Pre-rendered Artifacts (distribution/)
...
.. code-block::
    ...
    Pre-rendered Artifacts (distribution/) +
    Skills (distribution/per-project/general/skills/) +
    Instructions (distribution/per-project/general/instructions/) →
    agentsmgr populate →
